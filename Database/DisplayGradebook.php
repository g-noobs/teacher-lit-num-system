    ...
    function gradebookData(){
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $topicsTakenQuery = "SELECT COUNT(DISTINCT lsp.story_id) AS total_topics FROM tbl_learner_story_progress lsp
                    ...
                    WHERE lsp.learner_id = '$learnerId' AND t.topic_status = 1 AND added_byID = '$teacher_id'";
            ...
            $totalTopicsQuery = "SELECT COUNT(DISTINCT topic_id) AS total_topics FROM tbl_topic WHERE topic_status = 1 AND added_byID = '$teacher_id'";
            ...
            $quizTakenQuery = "SELECT learner_id, COUNT(DISTINCT quiz_id) AS total_quizzes FROM tbl_learner_quiz_progress WHERE learner_id = '$learnerId' AND quiz_id IN (SELECT quiz_id FROM tbl_quiz WHERE quiz_status = 1 AND added_byID = '$teacher_id')";
            ...
            $totalAssignmentQuery = "SELECT COUNT(DISTINCT assignment_id) AS total_assignment FROM tbl_assignment WHERE status  = 1 AND created_by = '$teacher_id'";
